...
from apps.api.accounts.exceptions import UserNotFoundAPIException
from apps.api.base.serializers import BaseSerializer
...
class LoginSerializer(BaseSerializer):
    ...
    def validate_email(self, value):
        if value:
            ...
            #     raise FirstTimePasswordAPIException(user=user)
...
class VerifyOTPNumberSerializer(BaseSerializer):
    ...
    def validate(self, attrs):
        ...
        otp_instance = auth_models.OTPNumber.objects.filter(user=user.first(), number=attrs["otp"])
...
class ForgetPasswordSerializer(BaseSerializer):
    ...
    def validate(self, attrs: dict):
        ...
        user = self.context["request"].user
        otp_instance = auth_models.OTPNumber.objects.filter(user=user, number=attrs["otp"])
        ...
        if not otp_instance.first().is_verified:
            raise exceptions.OTPNotVerifiedAPIException()
